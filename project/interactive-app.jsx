...
function initialState() {
  return {
    ...
    settings: {
      ...
      autoShareCritical: false,
    },
  };
}

const AUTO_SHARE_TYPES = new Set([
  'package_taken', 'weapon_detected', 'multiple_loitering',
]);

function AppInner() {
  const [state, setState] = React.useState(initialState());
  const [communityPosts, setCommunityPosts] = React.useState(COMMUNITY_POSTS);
  const [amazonDemoEvent, setAmazonDemoEvent] = React.useState(null);
  const sharedEventIds = React.useRef(new Set());
  ...
  // Share a Peep event into the neighborhood feed. Dedups by event id.
  const shareEventToCommunity = React.useCallback((event, { auto = false } = {}) => {
    if (!event || sharedEventIds.current.has(event.id)) return;
    sharedEventIds.current.add(event.id);
    const post = buildEventPost(event, { auto });
    setCommunityPosts(ps => [post, ...ps]);
    window.PeepRT?.sendPost?.(post);
    toast(
      auto ? 'Auto-shared clip with West End.' : 'Shared clip with West End.',
      { icon: 'broadcast' }
    );
  }, [toast]);
  ...
  const fireTestAlert = () => {
    ...
    // Auto-share to neighborhood if enabled and event qualifies
    if (state.settings.autoShareCritical && AUTO_SHARE_TYPES.has(ev.type)) {
      shareEventToCommunity(ev, { auto: true });
    }
    // Auto-dismiss banner
    setTimeout(() => {
  ...
  // Demo: always fires a package_taken event and auto-opens the cinematic Amazon agent pane.
  const fireAmazonDemo = () => {
    const ev = {
      ...
      conf: 0.94,
      receipt: genReceipt(),
      metadata: { source: 'demo', order_id: '114-8829112-0034221', carrier: 'Amazon' },
    };
    setState(s => ({
      ...s,
      activeEvent: ev,
      tab: 'live', detailStack: [],
      activeCamera: 'front', rewindOffset: 0,
      pendingNotification: ev,
    }));
    // Give the user a beat to see the Live screen before the Amazon pane slides up.
    setTimeout(() => setAmazonDemoEvent(ev), 1600);
    setTimeout(() => {
      setState(s => s.pendingNotification?.id === ev.id ? { ...s, pendingNotification: null } : s);
    }, 5000);
  };
  ...
  let mainScreen;
  if (state.tab === 'live') {
    mainScreen = <LiveScreen state={state} setState={setState}
                   onOpenEvent={openEvent}
                   onShareToCommunity={shareEventToCommunity} />;
    ...
    mainScreen = <CommunityScreen posts={communityPosts} setPosts={setCommunityPosts} />;
  } else if (state.tab === 'inbox') {
    mainScreen = <InboxScreen state={state} setState={setState} onReceived={handleDeliveryReceived} />;
  } else {
    mainScreen = <SettingsScreen state={state} setState={setState}
                   onFireTestAlert={fireTestAlert} onFireAmazonDemo={fireAmazonDemo} />;
  }
  ...
  return (
    <ExpertModeContext.Provider value={!!state.settings.expertMode}>
      <div style={{ width: '100%', height: '100%', position: 'relative' }}>
        ...
        {amazonDemoEvent && (
          <AmazonAgentDemo
            event={amazonDemoEvent}
            onClose={() => setAmazonDemoEvent(null)}
          />
        )}
      </div>
...
function PushNotificationBanner({ event, onTap, onDismiss }) {
  ...
  }[event.type] || `${event.time}. Tap to view.`;

  return (
    <div style={{
      position: 'absolute', left: 12, right: 12, top: 56, zIndex: 200,
      animation: 'peepBannerDrop .35s cubic-bezier(.2,.7,.3,1) both',
    }}>
      <Press onTap={onTap} style={{
        background: peep.surface, color: peep.text,
        ...
        <div style={{ fontSize: 14, fontWeight: 500, color: peep.text, marginTop: 2 }}>{headline}</div>
        <div style={{ fontSize: 13, color: peep.textSec, lineHeight: 1.4 }}>{subhead}</div>
      </Press>
    </div>
  );
}
