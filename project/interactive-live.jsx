      ...
      if (running && v && v.readyState >= 2 && v.videoWidth > 0) {
        try {
          ...
          const PKG = new Set([
            ...
            'toaster', 'refrigerator', 'dining table', 'bench',
          ]);
          const mapped = [];
          for (const p of preds) {
            const xPct = (p.bbox[0] / W) * 100;
            const yPct = (p.bbox[1] / H) * 100;
            const wPct = (p.bbox[2] / W) * 100;
            const hPct = (p.bbox[3] / H) * 100;
            const isPkgClass = PKG.has(p.class);
            const isPersonish = p.class === 'person';
            if (!isPkgClass && !isPersonish) continue;
            if (isPkgClass) {
              const aspect = wPct / Math.max(hPct, 0.01);
              const area = wPct * hPct;
              if (aspect < 0.45 || aspect > 1.85) continue;
              if (area < 2.0 || area > 55.0) continue;
            }
            mapped.push({
              x: xPct, y: yPct, w: wPct, h: hPct,
              label: isPkgClass ? 'package' : p.class,
              conf: p.score.toFixed(2),
              color: isPersonish ? '#7ea582' : isPkgClass ? '#706b8e' : '#c8a86a',
            });
          }
          setDets(mapped);
        } catch {}
