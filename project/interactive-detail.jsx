          ...
          return (
            <Press key={e.id} onTap={() => onOpen(e)} style={{
              ...
              <div style={{
                ...
                <Icon name={m.icon} size={18} color={c} weight="fill" />
              </div>
...
// ───────── Event detail screen ─────────
function EventDetailScreen({ ev, onBack, onOpenEvent, onShareToCommunity }) {
  ...
  const [didShare, setDidShare] = React.useState(false);
  const shareable = ['package_taken', 'weapon_detected', 'person_loitering',
    'multiple_loitering', 'after_hours_activity'].includes(ev.type);
    ...
    <Screen hideTitle backTitle="Activity" onBack={onBack}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <Card style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {techExpanded && (
              <div style={{
                ...
                <div><span style={{ color: '#bf7af0', fontWeight: 600 }}>action_layer</span> · {browserLine(ev.type, receipt)}</div>
              </div>
            )}
          </Card>
        </div>
        ...
        <div style={{ padding: '0 20px 8px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <ActionRow icon="check"            title="Mark resolved"
            onTap={() => toast('Marked resolved.', { icon: 'check' })} />
          <ActionRow icon="warning-circle"   title="Report wrong"
            onTap={() => toast('Reported. Peep will retrain on this clip.', { icon: 'arrow-clockwise' })} />
          <ActionRow icon="arrow-up-right"   title="Share clip"
            onTap={() => toast('Clip link copied.', { icon: 'link' })} />
          {shareable && onShareToCommunity && (
            <ActionRow icon="broadcast"
              title={didShare ? 'Shared with neighbors' : 'Share with neighbors'}
              onTap={() => {
                if (didShare) return;
                onShareToCommunity(ev);
                setDidShare(true);
              }} />
          )}
          {m.severity === 'critical' && (
            <ActionRow icon="phone" title="Call police" destructive
              onConfirm={() => toast('Dialing 911…', { icon: 'phone', accent: peep.critical })} />
          )}
        </div>
      </div>
    </Screen>
